...
class ConvLayer(BasicLayer):
    ...
    def weight_call(self, x):
        N, C, H, W = x.shape
        H2 = math.ceil(H / self.stride)
        W2 = math.ceil(W / self.stride)
        Pr = (H2 - 1) * self.stride + (self.kernel_size - 1) * self.dilation + 1 - H
        Pc = (W2 - 1) * self.stride + (self.kernel_size - 1) * self.dilation + 1 - W
        x = torch.nn.ZeroPad2d((Pr // 2, Pr - Pr // 2, Pc // 2, Pc - Pc // 2))(x)
        x = self.conv(x)
