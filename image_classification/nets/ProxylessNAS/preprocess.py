...
class TFPreprocessEval():
    def __init__(self, workers):
        self.sesses = [None] * workers
        self.workers = workers

    def tf_preprocess_transform(self, image):
        pid = os.getpid() % self.workers
        sessXY = self.sesses[pid]
        if sessXY is None:
            import tensorflow as tf
            ...
            # config.gpu_options.visible_device_list = str(pid)
            with sess.as_default():
                with graph.as_default():
                    X = tf.placeholder(dtype=tf.uint8,
                                    shape=(None, None, 3))
                    Y = preprocess_for_eval(X, 224, 224)
            self.sesses[pid] = (sess, X, Y)
        else:
            sess, X, Y = sessXY
        image_array = np.asarray(image)[:, :, 0:3]
        processed_image = sess.run(Y, feed_dict={X: image})
        ...
        if sessXY is None:
            ...
            with sess.as_default():
                with graph.as_default():
                    X = tf.placeholder(dtype=tf.uint8,
                                       shape=(None, None, 3))
                    Y = preprocess_for_train(X, 224, 224, None)
            self.sesses[pid] = (sess, X, Y)
        else:
            sess, X, Y = sessXY
        image_array = np.asarray(image)[:, :, 0:3]
